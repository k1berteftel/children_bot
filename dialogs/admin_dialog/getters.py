...
async def get_static(clb: CallbackQuery, widget: Button, dialog_manager: DialogManager):
    ...
    text = (f'<b>Статистика на {datetime.today().strftime("%d-%m-%Y")}</b>\n\nВсего пользователей: {len(users)}'
            ...
            f'\n - Позавчера: + {entry.get("2_day_ago")}\n\n<b>Доходы:</b>\n - Заработано всего: {static.sum}\n - '
            f'Всего покупок: {static.buys}\n - Покупок "Развивашки": {static.child_buys}\n - Покупок "Рецепты": '
            f'{static.recipe_buys}\n'
            f' - Покупок "Рецепты" + "Развивашки": {static.buys - static.child_buys - static.recipe_buys}')
...
async def deeplink_menu_getter(dialog_manager: DialogManager, **kwargs):
    ...
    text = (f'<b>({deeplink.name}) 🗓 Cоздано: {datetime.today().strftime("%d-%m-%Y")}</b>\n\nОбщее:\nВсего: {len(users)}'
            ...
            f'<b>🔗 Ссылка:</b> <code>https://t.me/childrenhelprobot?start={deeplink.link}</code>')
    return {'text': text}
